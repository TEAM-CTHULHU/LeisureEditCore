...
var BasicOptions, EditCore, Fragment, Headline, OrgEditing, Results, SimpleMarkup, Source, blockAttrs, blockLabel, contentSpan, escapeAttr, escapeHtml, last, numSpan, orgDoc, parseOrgMode, ref, ref1,
...
OrgEditing = (function(superClass) {
  ...
  OrgEditing.prototype.moved = function(editor) {
    ...
    if (blockId) {
      ...
      cur = blockId;
      while (block.prev) {
        block = this.blocks[block.prev];
        line += block.text.split('\n').length - 1;
      }
      $("#status").html("Block: " + blockId + (numSpan('')) + " line: " + (numSpan(line)) + " col: " + (numSpan((ref3 = (ref4 = last(lines)) != null ? ref4.length : void 0) != null ? ref3 : 0)) + " block line: " + (numSpan(lines.length)));
      return;
    }
