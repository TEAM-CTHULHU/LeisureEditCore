...
var Drawer, HTML, Headline, Keyword, Meat, Results, Source, _L, checkMerged, checkSingleNode, createChildrenDocs, createCodeBlockDoc, createDocFromOrg, createHtmlBlockDoc, createOrgDoc, crnl, docRoot, dump, getCodeItems, getSourceNodeType, isCodeBlock, isMergeable, isSourceEnd, isYaml, lineCodeBlockType, linkDocs, orgDoc, parseOrgMode, ref, ref1, replaceOrgDoc, root, safeLoad;

root = (ref = typeof require === "function" ? require('./org') : void 0) != null ? ref : Org;
...
ref1 = typeof Yaml !== "undefined" && Yaml !== null ? Yaml : typeof require === "function" ? require('./yaml') : void 0, safeLoad = ref1.safeLoad, dump = ref1.dump;
...
docRoot = function(collection) {
  var ref2, ref3;
  return (ref2 = ((ref3 = collection.leisure) != null ? ref3 : collection.leisure = {}).info) != null ? ref2 : (collection.leisure.info = collection.findOne({
    info: true
  }));
};
...
createOrgDoc = function(org, local) {
  var children, next, ref2, ref3, result;
  next = org.next;
  if (org instanceof Headline) {
    ...
    ref2 = createHtmlBlockDoc(org), result = ref2[0], next = ref2[1];
  } else if (isCodeBlock(org)) {
    ref3 = createCodeBlockDoc(org), result = ref3[0], next = ref3[1];
  } else {
    result = _L([
      {
        text: org.allText(),
...
createChildrenDocs = function(org, local) {
  var child, childDoc, children, mergedText, offset, ref2, ref3, ref4;
  children = _L();
  ...
  if (child) {
    ...
    while (child) {
      if (isMergeable(child)) {
        ...
        ref2 = checkMerged(mergedText, children, offset), mergedText = ref2[0], children = ref2[1];
        ref3 = createOrgDoc(child, local), childDoc = ref3[0], child = ref3[1];
        children = children.concat([childDoc]);
        offset = child != null ? child.offset : void 0;
      }
    }
    ref4 = checkMerged(mergedText, children, offset), mergedText = ref4[0], children = ref4[1];
  }
  return children;
};
...
createCodeBlockDoc = function(org) {
  var err, expected, first, firstOffset, l, last, name, obj, ref2, ref3, results, source, text;
  text = '';
  ref2 = getCodeItems(org), first = ref2.first, name = ref2.name, source = ref2.source, last = ref2.last, expected = ref2.expected, results = ref2.results;
  if (!first) {
    return [
      _L([
        {
    ...
    if (((ref3 = obj.codeAttributes) != null ? ref3.local : void 0) != null) {
      obj.local = true;
    }
    if (l = source.lead()) {
      obj.language = l.trim();
