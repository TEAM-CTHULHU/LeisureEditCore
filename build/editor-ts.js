...
import { useEvent, defaultBindings, last } from './editor.js';
...
export class BasicEditingOptionsNew extends Observable {
    ...
    nodeForId(id) { return $(`#${id}`); }
    idForNode(node) { return $(node).prop('id'); }
...
export function sameCharacter(c1, c2) {
    return c1 == c2 || (spaces.includes(c1) && spaces.includes(c2));
}
export function computeNewStructure(access, oldBlocks, newText) {
...
export function set$(new$, is$Func) {
    $func = new$;
    is$ = is$Func;
}
/**
...
function checkFJQValue(expected, actual) {
    if (expected !== actual) {
        if (expected instanceof Element && actual instanceof Element
            ...
            && expected.outerHTML == actual.outerHTML)
            return;
        throw new Error('bad FeatherJQ result');
    }
}
function checkFJQ(expected, actual) {
    if (!(expected instanceof jQuery))
        return checkFJQValue(expected, actual);
 ...
 */
function vFeatherJQ(args, context = document) {
    if (args.vfProxyValue)
        args = args.vfProxyValue;
    ...
    return new Proxy(f$(args, context), {
        get(_target, prop, _receiver) {
            if (prop === Symbol.iterator)
                return fjq[Symbol.iterator];
            if (prop === 'vfProxyValue')
                return fjq;
            if (['ready', 'data', 'context'].includes(prop))
                return fjq[prop];
...
//# sourceMappingURL=editor-ts.js.map