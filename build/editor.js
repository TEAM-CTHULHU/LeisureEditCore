...
var BS, BasicEditingOptions, DEL, DOWN, DataStore, DataStoreEditingOptions, END, ENTER, HOME, LEFT, LeisureEditCore, Observable, PAGEDOWN, PAGEUP, RIGHT, TAB, UP, _to_ascii, blockText, defaultBindings, getEventChar, htmlForNode, idCounter, isEditable, keyFuncs, last, link, maxLastKeys, modifiers, modifyingKey, root, selectRange, shiftKey, shiftUps, specialKeys,
  slice = [].slice,
...
LeisureEditCore = (function(superClass) {
  ...
  LeisureEditCore.prototype.handleInsert = function(e, s, text, select) {
    var blocks, cur, end, endHolder, endId, endPos, start, startBlock, startHolder, startPos;
    e.preventDefault();
    startHolder = this.options.getContainer(s.anchorNode);
    startBlock = this.options.getBlock(this.options.idForNode(startHolder));
    blocks = [startBlock];
    endPos = startPos = this.getTextPosition(startHolder, s.anchorNode, s.anchorOffset);
    if (s.type === 'Range') {
      endHolder = this.options.getContainer(s.anchorNode);
      endId = this.options.idForNode(endHolder);
      cur = startBlock;
      while (cur._id !== endId) {
        blocks.push(cur = this.options.getBlock(cur.next));
      }
      start = this.domCursor(s.anchorNode, s.anchorOffset);
      end = this.domCursor(s.focusNode, s.focusOffset);
      endPos = startPos + start.countChars(end);
    }
    return this.editBlocks(blocks, startPos, endPos, text != null ? text : getEventChar(e), select);
  };
  ...
  LeisureEditCore.prototype.cutText = function(e) {
    ...
    if (sel.type === 'Range') {
      ...
      text = this.domCursor(sel.anchorNode, sel.anchorOffset).getTextTo(this.domCursor(sel.focusNode, sel.focusOffset));
      this.options.simulateCut({
        html: html,
        text: text
      });
      return this.handleInsert(e, sel, '');
    }
  };
  ...
  LeisureEditCore.prototype.editBlocks = function(blocks, start, end, newContent, select) {
    var caret, endPos, newBlocks, newText, next, offset, oldBlocks, oldText, r, ref, startPos;
    ...
    if (select) {
      ...
      return startPos.forwardChars(start + offset + newContent.length, true).moveCaret();
    }
  };
    ...
    this.node.on('cut', (function(_this) {
      return function(e) {
        ...
        if (sel.type === 'Range') {
          ...
          clipboard.setData('text/plain', _this.domCursor(sel.anchorNode, sel.anchorOffset).getTextTo(_this.domCursor(sel.focusNode, sel.focusOffset)));
          return _this.handleInsert(e, sel, '');
        }
      };
    })(this));
      ...
      return function(e) {
        ...
        if (bound) {
          ...
          if (c === ENTER) {
            ...
            return _this.handleInsert(e, s, null, false);
          }
        }
      };
    })(this));
