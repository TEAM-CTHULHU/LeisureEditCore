...
var BS, BasicOptions, Changes, DEL, DOWN, END, ENTER, EditCore, HOME, LEFT, PAGEDOWN, PAGEUP, RIGHT, TAB, UP, defaultBindings, getEventChar, keyFuncs, last, maxLastKeys, modifiers, modifyingKey, root, selectRange, shiftKey, shiftUps, specialKeys, _to_ascii;
...
EditCore = (function() {
  function EditCore(node, options) {
    this.node = node;
  ...
  EditCore.prototype.backspace = function(event, sel, r) {
    ...
    this.currentBlockIds = [(this.getBlock(holderId))._id];
  ...
  EditCore.prototype.handleDelete = function(e, s, forward, delFunc) {
    ...
    if (s.type === 'Caret') {
      ...
      if (!result) {
        ...
        if (pos < 0) {
          if (blocks.prev) {
            blocks.push(bl = this.getBlock(block.prev));
            pos += bl.text.length;
            stop += bl.text.length;
          } else {
            return;
  ...
  EditCore.prototype.editBlock = function(blocks, start, end, newContent, caret) {
    var bl, block, i, newPrev, newText, oldText, prev, prevHolder, save, saveC, _i;
    oldText = ((function() {
    ...
    if (caret != null) {
      ...
      for (i = _i = 0; _i < 2; i = ++_i) {
        if (newPrev = this.getBlock(prev.prev)) {
          prev = newPrev;
  ...
  EditCore.prototype.changeStructure = function(oldBlocks, newText) {
    var fo, lo, newBlocks;
    this.changes = new Changes(this.options);
    newBlocks = this.options.parseBlocks(newText);
    this.checkMerge(fo = oldBlocks[0], newBlocks[0], this.getBlock(fo.prev), function(aux) {
      return aux + newBlocks.shift().text;
    });
    if (newBlocks.length) {
      this.checkMerge(lo = last(oldBlocks), last(newBlocks), this.getBlock(lo.next), function(aux) {
        return newBlocks.pop().text + aux;
      });
    }
  ...
  EditCore.prototype.checkMerge = function(oldBlock, newBlock, neighbor, func) {
    if (this.options.isMergeable(newBlock, neighbor, oldBlock)) {
      neighbor.text = func(neighbor.text);
      this.changes.updateBlock(neighbor);
      return neighbor._id;
    } else {
      return oldBlock._id;
    }
  ...
  EditCore.prototype.remapBlocks = function(oldBlocks, newBlocks) {
    var block, deleteCount, diff, diffs, i, insertCount, newTypes, offset, oldTypes, prevId, updateCount, _i, _j, _k, _len;
    oldTypes = (function() {
      var _i, _len, _results;
      _results = [];
      for (_i = 0, _len = oldBlocks.length; _i < _len; _i++) {
        block = oldBlocks[_i];
        _results.push(block.type);
      }
      return _results;
    })();
    newTypes = (function() {
      var _i, _len, _results;
      _results = [];
      for (_i = 0, _len = newBlocks.length; _i < _len; _i++) {
        block = newBlocks[_i];
        _results.push(block.type);
      }
      return _results;
    ...
    for (_i = 0, _len = diffs.length; _i < _len; _i++) {
      ...
      for (i = _j = 0; 0 <= deleteCount ? _j < deleteCount : _j > deleteCount; i = 0 <= deleteCount ? ++_j : --_j) {
        this.changes.removeBlock(oldBlocks.pop());
      }
      for (i = _k = 0; 0 <= insertCount ? _k < insertCount : _k > insertCount; i = 0 <= insertCount ? ++_k : --_k) {
  ...
  EditCore.prototype.bind = function() {
    ...
    return this.node.on('keydown', (function(_this) {
      return function(e) {
        ...
        _ref = _this.findKeyBinding(e, r), bound = _ref[0], checkMod = _ref[1];
        if (bound) {
  ...
  EditCore.prototype.addKeyPress = function(e, c) {
    ...
    if (notShift = !shiftKey(c)) {
      ...
      for (i = _i = 0, _ref = Math.min(this.lastKeys.length, maxLastKeys); 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
  ...
  EditCore.prototype.moveDown = function() {
    var line, linePos, pos, prev, _ref;
    linePos = prev = pos = this.domCursorForCaret().save();
    if (!((_ref = this.prevKeybinding) === keyFuncs.nextLine || _ref === keyFuncs.previousLine)) {
...
Changes = (function() {
  ...
  Changes.prototype.getChangedBlock = function(id) {
    var _ref;
    return (_ref = this.updates[id]) != null ? _ref : this.options.getBlock(id);
  };

  Changes.prototype.getUpdateBlock = function(id) {
    var _ref;
    return (_ref = this.updates[id]) != null ? _ref : (this.updates[id] = this.getCopy(id));
  };

  Changes.prototype.getOldBlock = function(id) {
    var _ref;
    return (_ref = this.oldBlocks[id]) != null ? _ref : this.options.getBlock(id);
  };
...
BasicOptions = (function() {
  ...
  BasicOptions.prototype.setEditor = function(editor) {
    this.editor = editor;
  };
