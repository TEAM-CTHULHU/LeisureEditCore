...
Basic Idea
==========
This edits a list of "blocks" that can partially render itself as DOM nodes.
...
* EXTRA STUFF: other things you might want to store in a block
...
DecentEditor
============
new DecentEditor node, options

options (BasicOptions provides all except for load(text) and edit(func)):
...
* bindings: a map of bindings (can use DecentEditor.defaultBindings)
...
* isMergeable(newBlock, oldBlock, neighbor):
  true if newBlock and oldBlock have different types and newBlock can merge with neighbor
* load(el, text): parse text into blocks and replace el's contents with rendered DOM
* getFirst(): get the first block id
* edit(func):
  1. prepare to edit
  2. run editing func which returns {removes: (map of id->true), updates: (map of id->block)}
  3. render changes into DOM, altering editor contents
    ...
    class DecentEditor
      ...
      checkMerge: (oldBlock, newBlock, neighbor, func)->
        if @options.isMergeable newBlock, oldBlock, neighbor
...
BasicOptions is an opinionated default options class that encourages using a "data-type" attribute to mark blocks in the DOM and a "data-noncontent" attribute to mark items that are not part of the content.
    ...
    class BasicOptions
      ...
      isMergeable: (el, text)-> throw new Error "options.isMergeable(newBlock, oldBlock, neighbor) is not implemented"
      load: (el, text)-> throw new Error "options.load(el, text) is not implemented"
    ...
    if window? then window.DecentEditor = root else module.exports = root
